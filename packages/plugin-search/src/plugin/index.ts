 ...
 * @module server/index
 *
 * 搜索插件的服务端入口，定义 VitaPressPlugin 实例。
 ...
 *
 * 索引交付方式：
 * 通过 Vite 虚拟模块 `virtual:vita-press-search/index` 提供搜索索引，
...
const VIRTUAL_SEARCH_INDEX_ID = 'virtual:vita-press-search/index'
 ...
 * 客户端通过 `import('virtual:vita-press-search/index')` 按需加载。
 *
 * @param _options - 插件配置选项（预留扩展，当前未使用）
 * @returns VitaPress 插件实例
 */
export function searchPlugin(_options: SearchPluginOptions = {}): VitaPressPlugin {
  ...
  return {
     ...
     */
    appCreated(app: VitaPressApp): void {
      appRef = app
    },
    /**
     * Markdown 解析完成后回调
